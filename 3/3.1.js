...
const parsedNumbers = numbers.map((num) => {
    ...
    return [...num, idxToSearch, characters.map((i) => i.join('')), !!valid, special.map(i => JSON.stringify(i))]
})
...
const gears = Object.values(gearsObject).filter(i => i.length > 1 && i[0][7].length > 0)

const gearRatio = gears.reduce((acc, curr) => acc + parseInt(curr[0]) * parseInt(curr[1]), 0)
